...
- Name: auth\login.spec.ts >> Login & Session Management Tests @login >> TC-AUTH-04: Login with username only (password blank) @regression
- Location: src\tests\auth\login.spec.ts:36:7
...
Locator:  locator('.oxd-input-group:has(input[name="password"]) .oxd-input-field-error-message')
Expected: "Required"
Received: "需要"
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('.oxd-input-group:has(input[name="password"]) .oxd-input-field-error-message')
    22 × locator resolved to <span data-v-7b563373="" data-v-957b4417="" class="oxd-text oxd-text--span oxd-input-field-error-message oxd-input-group__message">需要</span>
       - unexpected value "需要"
