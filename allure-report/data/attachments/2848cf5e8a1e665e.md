...
- Location: src\tests\pim.spec.ts:40:5
...
Expected substring: "QA Engineer"
Timeout: 10000ms
...
> 44 |     await expect(pimPage.tableRows.first()).toContainText(testData.adminModule.newJobTitle);
  ...
  45 | });
  46 | 
  47 | test('TC-PIM-07-Delete an employee @regression', async ({ pimPage }) => {
  48 |     await pimPage.deleteEmployee(testData.employee.employeeId);
  49 |     await pimPage.searchEmployeeById(testData.employee.employeeId);
  50 |     await expect(pimPage.tableBody).toContainText('No Records Found');
  51 | });
  52 | 
  53 | test('TC-PIM-08-Upload profile photo (valid JPG, under 1 MB) @regression', async ({ page, pimPage }) => {
  54 |     await pimPage.uploadProfilePhoto(testData.employee.employeeId, dummyJpgPath);
  55 |     await expect(page.locator('div.oxd-toast')).toBeVisible().catch(() => {});
  56 | });
  57 | 
  58 | test('TC-PIM-09-Upload unsupported file type as profile photo @regression', async ({ pimPage }) => {
  59 |     await pimPage.uploadProfilePhoto(testData.employee.employeeId, dummyTxtPath);
  60 |     await expect(pimPage.fileUploadErrorMessage).toBeVisible();
  61 |     await expect(pimPage.fileUploadErrorMessage).toContainText(/File type not allowed|Invalid/i);
  62 | });
  63 | 
  64 | test('TC-PIM-10-Add emergency contact for employee @regression', async ({ pimPage }) => {
  65 |     const contact = testData.employee.emergencyContact;
  66 |     await pimPage.addEmergencyContact(testData.employee.employeeId, contact.name, contact.relationship, contact.mobile);
  67 |     await expect(pimPage.emergencyContactsTable).toContainText(contact.name);
  68 |     await expect(pimPage.emergencyContactsTable).toContainText(contact.relationship);
  69 |     await expect(pimPage.emergencyContactsTable).toContainText(contact.mobile);
  70 | });
  71 | });
```